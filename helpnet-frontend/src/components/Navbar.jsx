...
// Replace the first two useEffects in Navbar.jsx with this:
useEffect(() => {
  const syncUser = () => {
    ...
    if (storedUser) {
      try {
        ...
        setUser(parsedUser);
        fetchUnreadCount(); // Only fetch count if user exists
      } catch (err) {
  ...
  // 1. Initial check on mount & when navigating
  syncUser();

  // 2. Listen for EVERY possible login event
  window.addEventListener("storage", syncUser);          // Multi-tab support
  window.addEventListener("authChange", syncUser);      // Our custom event
  window.addEventListener("local-storage-update", syncUser); // Just in case
      ...
      <div className="flex justify-between items-center px-6 py-4 max-w-7xl mx-auto relative">
        ...
        <div className="flex md:hidden items-center gap-2">
          ...
          >
            ...
            {!open && unreadCount > 0 && (
              <span className="absolute top-2 right-2 h-2.5 w-2.5 rounded-full bg-red-500 ring-2 ring-white dark:ring-gray-950"></span>
            )}
          </button>
        </div>
            ...
            {user ? (
              <>
                ...
                <div className="mt-3 grid grid-cols-1 gap-2">
                  <Link
                    to="/post"
                    className="bg-blue-600 text-white p-3.5 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg active:scale-95"
                  >
                    <PlusCircle size={18} /> Post Request
                  </Link>
                  <Link
                    to="/OfferHelp"
                    className="bg-cyan-50 dark:bg-cyan-900/20 text-cyan-600 dark:text-cyan-400 p-3.5 rounded-xl font-bold flex items-center justify-center gap-2"
                  >
                ...
                <button
                  onClick={handleLogout}
                  className="mt-3 flex items-center gap-3 px-4 py-3 text-red-500 font-bold text-sm hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors"
                >
                  <LogOut size={18} /> Sign Out
                </button>
              </>
              ...
              <Link
                to="/login"
                className="bg-gray-900 dark:bg-white text-white dark:text-gray-900 p-4 rounded-2xl font-bold text-center mt-2"
              >
                Sign In
              </Link>
            )}
...
function MobileLink({ to, icon, label, badge }) {
  return (
    <Link
      to={to}
      className="flex items-center justify-between px-4 py-3 rounded-xl text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
    >
      <div className="flex items-center gap-3">
        {icon}
