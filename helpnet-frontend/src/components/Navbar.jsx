    ...
    <nav className="sticky top-0 z-50 w-full backdrop-blur-xl bg-white/80 dark:bg-gray-950/80 border-b border-gray-200/50 dark:border-gray-800/50">
      <div className="flex justify-between items-center px-6 py-4 max-w-7xl mx-auto relative">
          ...
          <div className="flex items-center gap-4 border-l border-gray-200 dark:border-gray-800 pl-6">
            ...
            {user ? (
              <div className="flex items-center gap-3">
                {/* 💡 2. DESKTOP MAIL ICON */}
                <Link to="/dashboard?tab=messages" className="relative p-2 text-gray-500 hover:text-blue-600 dark:text-gray-400 transition-colors">
                  <Mail size={22} />
                  {unreadCount > 0 && (
              ...
              <Link to="/login" className="bg-gray-900 dark:bg-white text-white dark:text-gray-900 font-bold px-5 py-2 rounded-full">
                Sign In
              </Link>
            )}
          </div>
        ...
        <div className="flex md:hidden items-center gap-2">
          ...
          {/* 💡 3. MOBILE MAIL ICON (Always visible at the top if logged in) */}
          {user && (
            <Link to="/dashboard" className="p-2 text-gray-600 dark:text-gray-300 relative">
              <Mail size={22} />
              {unreadCount > 0 && (
                <span className="absolute top-1 right-1 h-2.5 w-2.5 bg-red-500 border-2 border-white dark:border-gray-950 rounded-full animate-pulse"></span>
              )}
          ...
          <button className="mobile-toggle p-2 text-gray-600 dark:text-gray-300 relative" onClick={() => setOpen(!open)}>
            {open ? <X size={28} /> : <Menu size={28} />}
          </button>
        </div>

        {/* Unified Mobile Dropdown Card */}
        <div ref={mobileMenuRef} className={`absolute top-20 right-4 left-4 md:hidden z-[100] transition-all duration-300 ease-in-out ${open ? "opacity-100 translate-x-0 pointer-events-auto" : "opacity-0 translate-x-10 pointer-events-none"}`}>
...
function DropdownItem({ to, icon, label, badge, highlight }) {
  ...
  return (
    <Link to={to} className={`flex items-center justify-between px-3 py-2.5 text-sm font-semibold transition-all ${highlight ? styles[highlight] : styles.default}`}>
      ...
      {badge > 0 && (
        <span className="bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-full font-bold">{badge}</span>
      )}
    </Link>
  );
}
