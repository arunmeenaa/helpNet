...
export default function Navbar() {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (mobileMenuRef.current && !mobileMenuRef.current.contains(event.target) && !event.target.closest(".mobile-toggle")) setOpen(false);
    };
    ...
    <nav className="sticky top-0 z-50 w-full backdrop-blur-xl bg-white/80 dark:bg-gray-950/80 border-b border-gray-200/50 dark:border-gray-800/50 transition-colors duration-300">
      <div className="flex justify-between items-center px-6 py-4 max-w-7xl mx-auto relative">
          ...
          <div className="flex items-center gap-4 border-l border-gray-200 dark:border-gray-800 pl-6">
            ...
            {user ? (
              <div className="flex items-center gap-3">
                {!isHomeless && (
                  <Link to="/dashboard?tab=messages" className="relative p-2 text-gray-500 hover:text-blue-600 transition-colors">
                    <Mail size={22} />
                    {unreadCount > 0 && <span className="absolute top-1 right-1 h-2.5 w-2.5 bg-red-500 border-2 border-white dark:border-gray-950 rounded-full animate-pulse"></span>}
                  </Link>
                )}
                ...
                <div className="relative" ref={dropdownRef}>
                  <button onClick={() => setDropdownOpen(!dropdownOpen)} className="flex items-center gap-2 p-1 pr-3 rounded-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 hover:border-blue-300 transition-all">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-600 to-cyan-500 text-white flex items-center justify-center text-xs font-bold">{user.fullName?.charAt(0).toUpperCase()}</div>
              ...
              <Link to="/choose-role" className="bg-gray-900 dark:bg-white text-white dark:text-gray-900 font-bold px-5 py-2 rounded-full">Sign In</Link>
            )}
          </div>
        </div>
        ...
        <div className="flex md:hidden items-center gap-2">
          <button onClick={() => setDark(!dark)} className="p-2">{dark ? <Sun size={22} className="text-yellow-500" /> : <Moon size={22} />}</button>
          {user && !isHomeless && (
            <Link to="/dashboard?tab=messages" className="p-2 relative">
              <Mail size={22} />
              {unreadCount > 0 && <span className="absolute top-1 right-1 h-2.5 w-2.5 bg-red-500 border-2 border-white rounded-full"></span>}
            </Link>
          )}
        ...
        {/* Mobile Dropdown */}
        <div ref={mobileMenuRef} className={`absolute top-20 right-4 left-4 md:hidden z-[100] transition-all duration-300 ${open ? "opacity-100 translate-x-0" : "opacity-0 translate-x-10 pointer-events-none"}`}>
          <div className="bg-white dark:bg-gray-950 border border-gray-100 dark:border-gray-800 rounded-3xl shadow-2xl p-4 flex flex-col gap-1">
            {user && (
              <div className="flex items-center gap-3 p-3 mb-2 bg-gray-50 dark:bg-gray-900/50 rounded-2xl">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-600 to-cyan-500 text-white flex items-center justify-center font-bold">{user.fullName?.charAt(0).toUpperCase()}</div>
                ...
                {!isHomeless && (
                  <>
                    ...
                    <div className="mt-3 grid grid-cols-1 gap-2">
                      <Link to="/post" className="bg-blue-600 text-white p-3.5 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg"><PlusCircle size={18} /> Post Request</Link>
                      <Link to="/OfferHelp" className="bg-cyan-50 dark:bg-cyan-900/20 text-cyan-600 p-3.5 rounded-xl font-bold flex items-center justify-center gap-2"><Inbox size={18} /> Offer Requests</Link>
                    </div>
                  </>
                )}
                <button onClick={handleLogout} className="mt-3 flex items-center gap-3 px-4 py-3 text-red-500 font-bold rounded-xl hover:bg-red-50 transition-colors"><LogOut size={18} /> Sign Out</button>
              </>
            ) : (
              <Link to="/login" className="bg-gray-900 dark:bg-white text-white p-4 rounded-2xl font-bold text-center mt-2">Sign In</Link>
            )}
          </div>
        </div>
      </div>
...
function MobileLink({ to, icon, label, badge }) {
  return (
    <Link to={to} className="flex items-center justify-between px-4 py-3 rounded-xl text-gray-700 dark:text-gray-200 hover:bg-gray-50 transition-colors">
