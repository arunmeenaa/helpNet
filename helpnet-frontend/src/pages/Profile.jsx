...
export default function Profile() {
  ...
  const EditableField = ({ label, value, fieldName, icon }) => {
    ...
    const isLocked = fieldName === 'email';
    
    return (
      <div className="group py-3 border-b border-gray-50 dark:border-gray-800/50 last:border-0">
        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">
          {label} {isLocked && <span className="ml-2 normal-case font-normal text-gray-500">(Verified)</span>}
        </label>
            ...
            <div className="flex-grow flex items-center gap-2 animate-in fade-in slide-in-from-left-2 duration-200 min-w-0">
              ...
              {fieldName === 'location' && (
                <button 
                  ...
                  className="p-1.5 shrink-0 bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-400 rounded-lg hover:scale-110 transition-transform"
                >
                  🪄
                </button>
              )}
              ...
              <button onClick={() => setEditingField(null)} className="p-1.5 shrink-0 bg-gray-200 dark:bg-gray-700 rounded-lg text-xs font-bold">✕</button>
            </div>
          ) : (
            <>
              ...
              <div className="flex items-center gap-2 min-w-0 overflow-hidden">
                ...
                <span className={`text-sm font-semibold truncate ${!value ? 'text-gray-400 italic' : isLocked ? 'text-gray-500 dark:text-gray-400' : 'text-gray-700 dark:text-gray-200'}`}>
                  {value || `Add ${label}`}
                </span>
              </div>
              ...
              {!isLocked ? (
                ...
                <span className="shrink-0 opacity-40 grayscale"><svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg></span>
              )}
            </>
          )}
        </div>
          ...
          <div className="pt-12 md:pt-16 pb-8 px-6 md:px-10">
            ...
            <div className="grid grid-cols-2 gap-3 md:gap-4 my-8">
              <div className="p-4 md:p-5 rounded-2xl md:rounded-3xl bg-gray-50 dark:bg-gray-800/40 border border-gray-100 dark:border-gray-800">
                <div className="text-xl md:text-2xl font-black text-blue-600">{stats.requests}</div>
                <div className="text-[8px] md:text-[9px] font-bold text-gray-400 uppercase tracking-widest">Requests</div>
              </div>
              <div className="p-4 md:p-5 rounded-2xl md:rounded-3xl bg-gray-50 dark:bg-gray-800/40 border border-gray-100 dark:border-gray-800">
                <div className="text-xl md:text-2xl font-black text-teal-600">{stats.offers}</div>
                <div className="text-[8px] md:text-[9px] font-bold text-gray-400 uppercase tracking-widest">Offers</div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-8 border-t border-gray-50 dark:border-gray-800">
              <button onClick={handleLogout} className="w-full sm:w-auto px-6 py-2.5 text-red-500 font-bold text-xs uppercase tracking-widest hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-all">
                Sign Out
              </button>
